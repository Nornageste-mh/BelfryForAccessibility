...
- 本补丁因此**不包含任何游戏资源**，只发布补丁代码；发布包（BepInEx 运行时 +
  补丁 + 说明）通过本仓库的 Releases 提供，其中**不含游戏本体、不含任何游戏资源**。
- 补丁不绕过任何技术保护措施，不修改、不替换、不再分发游戏文件。
- 证据目录 `probe/evidence/` 只收录**不含剧本文本**的材料：
  ...
  见 `.gitignore` 第 3 节。
- **再次请求：请通过 Steam 购买正版。** 这是一款由 可味玩KawayiPlay
  独立完成的作品，他们的劳动成果值得被正当地支持 ——
