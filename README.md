...
> 脚注（哪些是本仓库亲自核实的）：
>
> - **开发商**：取自游戏目录 `The Belfry_Data/app.info`，内容为 `Kawayi Play`，
>   与同厂的《透明的她与真实的我》是同一个 —— 这条是本机核实过的
> - **引擎**：`globalgamemanagers` 头部版本串 + `Managed/` 下 127 个托管程序集，
>   本机核实
> - **发售 / 试玩版日期、字数、配音覆盖**：来自公开资料（VNDB `v61157` 与商店页），
>   **本仓库未能直接核实** —— 开发环境里 Steam 商店页被网络策略拦截，
>   SteamDB 明确拒绝第三方抓取。本机能核实的是 2026 年 9 月 6 日的安装记录
>   与 `buildid` 23088782
...
> 试玩版（AppID 4169180）是同一个引擎版本，理论上同一套补丁也能用，
> 但**本仓库没有实机验证过**，所以不作承诺。
