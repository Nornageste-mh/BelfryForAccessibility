...
<#
    ...
    安装就是「把 package\ 里的东西拷进有 The Belfry.exe 的那一层」。
    手写说明见 package\安装说明.md。
    ...
    注意：本脚本必须保存为 UTF-8 带 BOM —— Windows PowerShell 5.1 读无 BOM 的
    脚本会按 GBK 解码，中文注释变乱码并直接语法错误。
#>
[CmdletBinding()]
param(
    ...
    # 复用本机已有的下载缓存（上一个仓库留下的），没有就自己下
    [string]$CacheDir = "D:\DSHWorkBase\transparenther_a11y\mod"
)
...
# ------------------------------------------------------------
Step "1/5  检查游戏目录"
if (-not (Test-Path -LiteralPath (Join-Path $GameDir "The Belfry.exe"))) {
...
if (Test-Path -LiteralPath (Join-Path $bepDir "BepInEx\core\BepInEx.dll")) {
    ...
    if (-not (Test-Path -LiteralPath $bepZip)) {
        $cacheZip = Join-Path $CacheDir "dl\BepInEx_win_x64_$BepInExVersion.zip"
        if (Test-Path -LiteralPath $cacheZip) {
            Copy-Item $cacheZip $bepZip -Force
            Ok "复用缓存 $cacheZip"
        } else {
...
if ($nvdaDll) {
    ...
    $cacheNvda = Join-Path $CacheDir "nvda_dl"
    if (Test-Path -LiteralPath $cacheNvda) {
        New-Item -ItemType Directory -Force -Path $nvdaDir | Out-Null
        Copy-Item (Join-Path $cacheNvda "*") $nvdaDir -Recurse -Force
        Ok "复用缓存 $cacheNvda"
    } else {
...
# 表现是模组完全没加载、游戏里一片安静，极难排查（上一作 v0.5.8a 踩过）。
# 想表达 0.5.8a 这种「第五版修订」，用第四位数字：0.5.8.1。
$srcDir  = Join-Path $mod "src\BelfryA11y"
$pluginCs = Join-Path $srcDir "Plugin.cs"
...
if (-not [System.Version]::TryParse($pluginVer, [ref]$parsed)) {
    ...
    Write-Host "    请改成纯数字形式，例如 0.1.0.1"
    exit 1
}
try {
    $asmVer = ([xml](Get-Content -LiteralPath (Join-Path $srcDir "BelfryA11y.csproj") -Raw)).Project.PropertyGroup.Version
...
try {
    dotnet build -c Release -v minimal -nowarn:MSB3277 -p:BelfryGameDir="$GameDir"
    if ($LASTEXITCODE -ne 0) { throw "编译失败" }
} finally { Pop-Location }
$pluginDll = Join-Path $srcDir "bin\Release\BelfryA11y.dll"
...
Write-Host "`n构建完成。安装：把 $pkg 里的东西全部拷进游戏根目录（有 The Belfry.exe 的那一层）。`n" -ForegroundColor Cyan
